...
const calculateLoan = asyncHandler(async (req, res) => {
    ...
    if (principal <= 0 || totalPayble <= 0 || roi <= 0 || tenure <= 0) {
    ...
    if (!user.isCompleteRegistration) {
        return res
            .status(400)
            .json({ message: "firstly please complete your registration" });
    }
    ...
    if (result.length > 0) {
        return res
            .status(400)
            .json({ message: `You have already ${result.length} active loan` });
    }

    let previousLoanApplication = await LoanApplication.findOne({
        userId: user._id,
    }).sort({ createdAt: -1 });
...
const addEmploymentInfo = asyncHandler(async (req, res) => {
    ...
    const missingFields = requiredFields.filter(
        (field) => !employeInfo[field] || employeInfo[field].trim() === ""
    );
    ...
    const loanDetails = await LoanApplication.findOne({ userId: userId }).sort({
        createdAt: -1,
    });
...
const disbursalBankDetails = sessionAsyncHandler(async (req, res, session) => {
    ...
    const loanDetails = await LoanApplication.findOne({ userId: userId })
        .sort({ createdAt: -1 })
        .session(session);
    ...
    const { fName, mName, lName } = splitName(
        userDetails.personalDetails.fullName
    );
    ...
    let leadBreCounter =
        breCounter < 5
            ? await nextSequence("breCounter")
            : await resetSequence("breCounter");
    ...
    const newLead = new Lead({
        ...
        alternateMobile: userDetails.alternateMobile
            ? String(userDetails.alternateMobile)
            : "",
        personalEmail: userDetails.personalDetails.personalEmail
            ? userDetails.personalDetails.personalEmail
            : "",
        officeEmail: loanDetails.employeeDetails.officeEmail
            ? loanDetails.employeeDetails.officeEmail
            : "",
        ...
        mothersName: userDetails.personalDetails.mothersName
            ? userDetails.personalDetails.mothersName
            : "",
        fathersName: userDetails.personalDetails.fathersName
            ? userDetails.personalDetails.fathersName
            : "",
        workingSince: userDetails.incomeDetails.workingSince
            ? userDetails.incomeDetails.workingSince
            : "",
        extraDetails,
    });
    ...
    if (newLead) {
        ...
        if (
            userDetails.incomeDetails.incomeMode === "CASH" ||
            userDetails.incomeDetails.incomeMode === "OTHERS"
        ) {
            newLead.isRejected = true;
            newLead.isRejectedBySystem = true;
            (updatedLoanDetails.applicationStatus = "REJECTED"),
            ...
            remarks.push(
                `User income mode is ${userDetails.incomeDetails.incomeMode}`
            );
        }
        ...
        if (
            ...
            remarks.push(
                `User monthly income is ${userDetails.incomeDetails.monthlyIncome}`
            );
        }
    ...
    return res
        .status(200)
        .json({ message: "Loan Applied successfully!", newLead, logs });
});
...
const getApplicationDetails = asyncHandler(async (req, res) => {
    ...
    if (!datas) {
        loanApplicationDetails = await LoanApplication.findOne({ userId }).sort(
            { createdAt: -1 }
...
const getDocumentStatus = asyncHandler(async (req, res) => {
    ...
    for (const [key, value] of Object.entries(multipleDocs)) {
        multipleDocumentsStatus[key] =
            value.length > 0 ? "Uploaded" : "Not Uploaded";
    }
...
const getDocumentList = asyncHandler(async (req, res) => {
    ...
    if (result) {
        ...
        for (const [key, docsArray] of Object.entries(multipleDocs)) {
            docsArray.slice(0, 3).forEach((doc) => {
                // Take only the first 3 documents of each type
...
const getJourney = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json({ message: "Loan Application journey found", journey: data });
});
